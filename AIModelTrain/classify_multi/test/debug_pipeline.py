...
用法（在 MultiClassify 目录下）:
  python test/debug_pipeline.py "这期节目太棒了，听着很开心"
  python test/debug_pipeline.py   # 使用默认测试句

输出：分词结果、token IDs、填充后的 [1,50] 序列、Python 预测结果及各类概率。
在 iOS 端用同一句测试，对比 token IDs 是否完全一致。
"""
import sys
import os
...
def main():
    text = sys.argv[1] if len(sys.argv) > 1 else "这期节目太棒了，听着很开心"
